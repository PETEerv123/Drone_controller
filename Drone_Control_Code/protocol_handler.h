...
class signalRx {
 public:
  ...
  #else
  explicit signalRx(HardwareSerial *bus) : uart_(bus) {}
  signalRx(HardwareSerial *bus, const bool inv) : uart_(bus), inv_(inv) {}
  signalRx(HardwareSerial *bus, const bool inv, const bool fast) : uart_(bus),
                                                                 inv_(inv),
                                                                 fast_(fast) {}
  #endif
  void Begin();
  bool Read();
  inline signalData data() const {return data_;}
 ...
 public:
  ...
  signalTx(HardwareSerial *bus, const int8_t rxpin, const int8_t txpin,
         const bool inv, const bool fast) : uart_(bus), inv_(inv), fast_(fast),
                                            rxpin_(rxpin), txpin_(txpin) {}
  #else
  explicit signalTx(HardwareSerial *bus) : uart_(bus) {}
  signalTx(HardwareSerial *bus, const bool inv) : uart_(bus), inv_(inv) {}
  signalTx(HardwareSerial *bus, const bool inv, const bool fast) : uart_(bus),
                                                                 inv_(inv),
                                                                 fast_(fast) {}
  #endif
  void Begin();
  void Write();
  inline void data(const signalData &data) {data_ = data;}
  inline signalData data() const {return data_;}
